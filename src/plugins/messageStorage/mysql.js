"use strict";

const log = require("../../log");
const Helper = require("../../helper");
const Msg = require("../../models/msg");

let mysql;

try {
	mysql = require("mysql");
} catch (e) {
	Helper.config.messageStorage = Helper.config.messageStorage.filter((item) => item !== "mysql");
	log.error("Unable to load mysql module.");
}

const currentSchemaVersion = 1520239200;

const schema = [
	// Schema version #1
	"CREATE TABLE IF NOT EXISTS options (user VARCHAR(255), name VARCHAR(255), value VARCHAR(255), CONSTRAINT name_unique UNIQUE (name))",
	"CREATE TABLE IF NOT EXISTS messages (user VARCHAR(255), network VARCHAR(255), channel VARCHAR(255), time BIGINT, type VARCHAR(255), msg TEXT)",
	"CREATE INDEX IF NOT EXISTS network_channel ON messages (network, channel)",
	"CREATE INDEX IF NOT EXISTS time ON messages (time)",
	"CREATE INDEX IF NOT EXISTS user ON messages (user)",
];

class MessageStorage {
	constructor(client) {
		this.client = client;
		this.isEnabled = false;
	}

	enable() {
		this.isEnabled = true;

		this.database = mysql.createPool({
			connectionLimit: 100,
			host: "localhost",
			user: "root",
			password: "secret",
			database: "thelounge", // this.client.name,
		});

		schema.forEach((line) => {
			this.database.query(line, function(error) {
				if (error) {
					log.error(line, error);
					throw error;
				}
			});
		});

		this.database.query(
			"SELECT value FROM options WHERE name = 'schema_version'",
			(err, row) => {
				if (err) {
					return log.error(`Failed to retrieve schema version: ${err}`);
				}

				// New table
				if (row === undefined) {
					this.database.query(
						"INSERT INTO options (name, value) VALUES ('schema_version', ?)",
						currentSchemaVersion
					);
					return;
				}

				const storedSchemaVersion = parseInt(row.value, 10);

				if (storedSchemaVersion === currentSchemaVersion) {
					return;
				}

				if (storedSchemaVersion > currentSchemaVersion) {
					return log.error(
						`mysql messages schema version is higher than expected (${storedSchemaVersion} > ${currentSchemaVersion}). Is The Lounge out of date?`
					);
				}

				log.info(
					`mysql messages schema version is out of date (${storedSchemaVersion} < ${currentSchemaVersion}). Running migrations if any.`
				);

				this.database.query(
					"UPDATE options SET value = ? WHERE name = 'schema_version'",
					currentSchemaVersion
				);
			}
		);
	}

	close(callback) {
		if (!this.isEnabled) {
			return;
		}

		this.isEnabled = false;

		this.database.end((err) => {
			if (err) {
				log.error(`Failed to close mysql database: ${err}`);
			}

			if (callback) {
				callback(err);
			}
		});
	}

	index(network, channel, msg) {
		if (!this.isEnabled) {
			return;
		}

		const clonedMsg = Object.keys(msg).reduce((newMsg, prop) => {
			// id is regenerated when messages are retrieved
			// previews are not stored because storage is cleared on lounge restart
			// type and time are stored in a separate column
			if (prop !== "id" && prop !== "previews" && prop !== "type" && prop !== "time") {
				newMsg[prop] = msg[prop];
			}

			return newMsg;
		}, {});

		this.database.query(
			"INSERT INTO messages(network, channel, time, type, msg, user) VALUES(?, ?, ?, ?, ?, ?)",
			[
				network.uuid,
				channel.name.toLowerCase(),
				msg.time.getTime(),
				msg.type,
				JSON.stringify(clonedMsg),
				this.client.name,
			],
			(err) => {
				if (err) {
					log.error(err);
					log.error("DATA", [
						network.uuid,
						channel.name.toLowerCase(),
						msg.time.getTime(),
						msg.type,
						JSON.stringify(clonedMsg),
					]);
				}
			}
		);
	}

	deleteChannel(network, channel) {
		if (!this.isEnabled) {
			return;
		}

		this.database.query("DELETE FROM messages WHERE network = ? AND channel = ? AND user = ?", [
			network.uuid,
			channel.name.toLowerCase(),
			this.client.name,
		]);
	}

	/**
	 * Load messages for given channel on a given network and resolve a promise with loaded messages.
	 *
	 * @param Network network - Network object where the channel is
	 * @param Chan channel - Channel object for which to load messages for
	 */
	getMessages(network, channel) {
		if (!this.isEnabled || Helper.config.maxHistory === 0) {
			return Promise.resolve([]);
		}

		// If unlimited history is specified, load 100k messages
		const limit = Helper.config.maxHistory < 0 ? 100000 : Helper.config.maxHistory;

		return new Promise((resolve, reject) => {
			this.database.query(
				"SELECT msg, type, time FROM messages WHERE network = ? AND channel = ? AND user = ? ORDER BY time DESC LIMIT ?",
				[network.uuid, channel.name.toLowerCase(), this.client.name, limit],
				(err, rows) => {
					if (err) {
						return reject(err);
					}

					resolve(
						rows
							.map((row) => {
								const msg = JSON.parse(row.msg);
								msg.time = row.time;
								msg.type = row.type;

								const newMsg = new Msg(msg);
								newMsg.id = this.client.idMsg++;

								return newMsg;
							})
							.reverse()
					);
				}
			);
		});
	}

	canProvideMessages() {
		return this.isEnabled;
	}
}

module.exports = MessageStorage;
